...
public class NavigationAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(context).inflate(R.layout.navigation_adapter,parent,false);
        return new NavigationHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ((NavigationHolder)holder).textView.setText(categoryLists.get(position).getCategory());
        Utils.setImage(((NavigationHolder) holder).image,context,categoryLists.get(position).getImgpath());
